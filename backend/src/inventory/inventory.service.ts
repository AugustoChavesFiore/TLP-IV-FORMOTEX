...
export class InventoryService implements IInventoryService {
    ...
    async createInventory(inventory: IInventory, idCategory: string, idOrganization: string): Promise<IInventory> {
        const newInventory = new Inventory({ ...inventory, category: new ObjectId(idCategory), organization: new ObjectId(idOrganization) });
    ...
    async updateInventory(id: string, inventory: IInventory): Promise<void> {
        ...
        if (updatedInventory.modifiedCount === 0) throw CustomError.NotFound('Inventory not found');
    };

    async deleteInventory(id: string): Promise<void> {
        const deletedInventory = await Inventory.deleteOne({ _id: id });
        if (deletedInventory.deletedCount === 0) throw CustomError.NotFound('Inventory not found');
    };
};
